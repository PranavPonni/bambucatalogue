...
(() => {
  ...
  const materialRange = p => p.weightEstimate.map(g => yen(g * 3.4)).join('–');
  ...
  const card = p => `<article class="product-card" data-id="${p.id}">
    ...
    <div class="price-block"><p class="price-label">EST. FILAMENT VALUE</p><p class="price">${materialRange(p)}</p><p class="price-note">Final selling price on enquiry</p></div>
  ...
  function render(){
    ...
    if(state.sort==='price-low') visible.sort((a,b)=>a.weightEstimate[0]-b.weightEstimate[0]);
    if(state.sort==='price-high') visible.sort((a,b)=>b.weightEstimate[0]-a.weightEstimate[0]);
